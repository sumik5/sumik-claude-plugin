...
**重要: 新規機能の実装前に必ずライブラリ調査と最新仕様確認を行ってください**

**調査・検索の優先順位：**

1. **`researching-libraries` スキル（新規ライブラリ導入時・必須）**: 既存ライブラリを調査して車輪の再発明を防止。npm/PyPI/pkg.go.dev等での比較検討を実施
2. **`searching-with-exa` スキル（技術情報検索・第一選択）**: Exa MCPによる7カテゴリ検索。WebSearchより検索精度が高いため、Web検索時はまずこちらを使用
3. **WebFetch**: 上記で見つけたURLの具体的なコンテンツ取得
4. **WebSearch**: Exa MCPが利用できない場合のフォールバック
5. **serena memory**: プロジェクト固有の設計決定やアーキテクチャ情報を参照
