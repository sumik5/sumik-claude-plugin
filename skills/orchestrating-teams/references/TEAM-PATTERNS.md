...
**チーム編成時、以下のスキルは全メンバー（planner/implementer問わず）のSpawn Promptに含める:**

- `researching-libraries`: 新規ライブラリ導入前の調査（車輪の再発明禁止）。plannerは技術選定時、implementerは実装中に新規依存が必要になった時に参照
- `searching-with-exa`: Web検索の第一選択（Exa MCP）。WebSearchより検索精度が高いため、技術情報・実装パターン・公式ドキュメントの検索時はまずこちらを使用
